...
/// <summary>One tab header of a strip band as the target catalog captured it: the live control plus its natural bounds in gesture coordinates (spec DA-9.7 v0.17).</summary>
internal readonly record struct StripHeaderView(DockTabHeader Header, Rect Rect);

/// <summary>
/// One strip band of the target catalog (spec DA-9.7 v0.17): the band control — a group's
...
/// <summary>
/// The live reorder-preview payload of one strip insertion zone (spec DA-9.7 v0.17): the
...
/// <summary>
/// Override engine of the live strip reorder preview (spec DA-9.7 v0.17): over a strip
...
/// the reference, which moves the actual TabLabel). The ghost-header rectangle — the gap the
/// headers opened — is computed here in gesture coordinates and rendered by the gesture
/// visual; it is clipped into the band, like the band clips its own overflowing headers
/// (overflow itself stays unhandled — document-area, section 11).
/// </summary>
internal sealed class StripReorderOverrides
{
    ...
    /// <summary>
    /// Applies the overrides of one strip zone and returns the ghost-header rectangle in
    /// gesture coordinates — empty when the band clips it away entirely. Idempotent per
    /// payload instance: the same zone of the same catalog re-applies nothing, while a
    /// rebuilt catalog delivers a fresh payload, which re-lays the overrides over the fresh
    /// headers (the reapply of the section 12 contract).
    /// </summary>
    public Rect Apply(StripReorderPreview preview, double ghostWidth)
    {
        ...
        Clear();
        _applied = preview;
        _ghostRect = LayOutReceiver(preview, ghostWidth);
    ...
    /// <summary>
    /// Lays the receiving band out as «the others plus the ghost»: the surviving headers and
    /// the ghost slot take consecutive positions from the natural start of the header run,
    /// each header shifted into its position; the dragged header collapses. The gap right
    /// after the dragged header's own position lays out identically to the natural order —
    /// the identity drop previews the tab staying in place (DA-E40).
    /// </summary>
    private Rect LayOutReceiver(StripReorderPreview preview, double ghostWidth)
    {
        ...
        {
            // The gap right after itself: the ghost takes the dragged header's own position.
        ...
        var x = template.X;
        var ghost = default(Rect);
        for (var i = 0; i <= others.Count; i++)
        {
            if (i == insertAt)
            {
                ghost = new Rect(x, template.Y, ghostWidth, template.Height);
                x += ghostWidth;
            }
        ...
        return ghost.Intersect(preview.Receiver.Rect);
    }
