...
/// defaults: keep it to reset the layout later without losing open documents —
/// <c>current.Apply(composition.State, ApplyScope.Arrangement, composition.Registry)</c>
/// restores the default placement and openness while the dock area stays untouched.
/// </summary>
public sealed class LayoutComposition
{
    ...
    /// <summary>The content coordinator over <see cref="Registry"/>; eager content is already created.</summary>
    public ContentLifecycle Lifecycle { get; }

    /// <summary>
    /// The initial layout: the default arrangement of the registered descriptors — the same
    /// rule as <see cref="LayoutApply.ResetToDefaults"/> — with the builder's initial commands
    /// applied on top: opened tool windows, documents and panel tabs.
    /// </summary>
