...
| **`LastInternalMode`** (с v0.5) | `internalType` + неизменность `isAutoHide` при уходе во Float/Window | та же память возврата одним явным полем (≈) |
...
| **`PairRatio` + выводимая доля пары (R1)** | **одно** поле `WindowInfoImpl.sideWeight` (дефолт `0.5`) на панель, плюс транзиентный кэш пропорции пары | со спеки v0.12 хранятся только предпочтения панелей, доля пары выводится нормализацией `P/(P+S)` — та же формула, что у прибытия Secondary эталона (см. дополнение к TW-2.7); поле `CurrentRatio` из ранних версий спеки удалено |
| **Толщина Undock-оверлея** | `weight` окна, при дефолте new UI затираемый весом стороны (`UnifiedToolWindowWeights`, см. дополнение к TW-3.3) | со спеки v0.12 — `Weight` стороны, без отдельного поля (`UndockWeight` ранних версий удалён): одно состояние вместо двух синхронизируемых |
| **`IsOpen`** | `isVisible` | |
...
2. **SideStack** (`SideStack.java`): IDEA умеет **восстановить** ранее вытесненную докированную панель при
   закрытии заменившей. Спека этого не делает — с v0.5 явная не-цель (раздел 12). В new UI выключено реестром
...
3. **HideAll** охватывает только `type.isInternal` (docked+sliding); Float/Window остаются открытыми
   (`HideToolWindowAction.kt:18`). Спека v0.5 приняла объём IDEA — снято.
4. **Сортировка списка «⋯»** — по мнемонике, затем по id алфавитно (`ToolWindowsGroup.java:78`).
   Спека v0.5 перешла на алфавит по `Title` (мнемоник у нас нет) — снято.
5. **Дефолт `Weight` = 0.33** у IDEA — спека v0.5 приняла 0.33, снято.
...
*Дополнение (2026-07, трассировка владельца к задаче 2.1): переполнение стрипа иконками (суммарная
высота сегментов больше высоты окна) IDEA не обрабатывает вовсе — ни скролла, ни сжатия иконок, ни
переноса лишних в «⋯»: необработанный вырожденный случай Swing-раскладки; эталон полагается на то,
что реалистичное число закреплённых панелей на сторону не превысит высоту экрана. Это пробел
эталона, а не поведение: бездействие Berth в том же случае — `=` (раздел 12 спеки).*
...
- `Weight` стороны: `UnifiedToolWindowWeights` — по одному `Float` на `top/left/bottom/right`
  (`UnifiedToolWindowWeights.kt:20-23`), дефолт `0.33` (спека v0.5 приняла тот же дефолт).
...
*Дополнение (2026-07, трассировка владельца к спеке v0.12): прочтение «сплиттер ставится из sideWeight
показанной» уточнено — `setSideWeight` вызывается **только** из `ToolWindowManagerImpl.setLayout`
(`:1380`, «PASS 2: adjust sizes», полная замена layout: Restore Default/Named Layout, инициализация
проекта), обычный show/activate одного окна этот путь не задействует. Живое образование пары идёт через
`addAndSplitDockedComponentCmd` (`ToolWindowPane.kt:611-737`) — см. дополнение к TW-2.7. Спека v0.12
сняла расщепление: `CurrentRatio` из состояния удалён, эффективная доля пары выводится нормализацией
предпочтений (R1) — вместо двух синхронизируемых состояний эталона одно выводимое значение.*
...
*Дополнение (2026-07, трассировка владельца к спеке v0.12) — оба тезиса о механике уточнены кодом:*

*R1: живое образование пары — `addAndSplitDockedComponentCmd` (`ToolWindowPane.kt:611-737`), развилка
по прибывшему (`:693-729`): прибывает **Primary** → `splitter.proportion =
normalizeWeight(info.sideWeight)`, вес существующего Secondary не читается; прибывает **Secondary** →
сначала транзиентный кэш `ToolWindowPaneState.idToSplitProportion` (ключ — id Primary; пишется при
скрытии Secondary из `removeDecorator`, при скрытии Primary — нет; не персистится, умирает с
пересозданием `ToolWindowPane`), иначе формула `normalizeWeight(oldInfo.sideWeight /
(oldInfo.sideWeight + info.sideWeight))` — нормализация, вырождающаяся в чистый `Primary.sideWeight`
при инварианте суммы = 1. Итог: у эталона результат зависит от порядка прибытия (прибывший Primary
диктует чисто, прибывший Secondary — нормализуется) — асимметрия двух независимых путей кода. Спека
v0.12 взяла формулу нормализации симметрично (эффективная доля пары — выводимая, функция от пары):
совпадает с эталоном при согласованной паре и при прибытии Secondary; расходится в одном случае —
прибывающий Primary с несогласованным весом (у нас нормализация, у IDEA чистое предпочтение).*

*R2: прежнее прочтение «пишется только двигаемая» неверно — каждый `ToolWindowImpl` вешает свой
`ComponentListener` (`ToolWindowImpl.kt:288-299`), при драге разделителя `Splitter` меняет bounds
**обоих** детей → `movedOrResized` вызывается для обоих и пишет `sideWeight` каждому
(`ToolWindowManagerDecorators.kt:163-189`). Симметричная запись и держит сумму пары ≈ 1 — наш R2
«учим обе» оказался `=`, а не `≈`. Кэш `idToSplitProportion` спека сознательно не берёт: при R2 веса
кодируют живую пропорцию точно и переживают рестарт (раздел 12 спеки).*
...
(=PairRatio), `weight:87` (у SLIDING = толщина оверлея; наш аналог со спеки v0.12 — `Weight` стороны, см. дополнение к TW-3.3), `floatingBounds:53`. На уровне
...
*Дополнение (2026-07, трассировка владельца к задаче 2.1): два одновременно видимых sliding-оверлея
одной стороны (Primary+Secondary, легально по INV-2) в живом потоке недостижимы — открытие второго
кликом по стрипу переносит фокус, и у первого срабатывает FOCUS_LOST → он закрывается тем же общим
автохайдом, что и DockUnpinned (TW-6.1), без привязки к слоту/стороне. Z-порядок в PALETTE_LAYER
существует как код, но практического пути к нему нет — случай «код есть, достижимости нет».
Спеке дополнение не требуется: TW-6.1 уже глобален и предсказывает поведение; z-порядок мимолётного
состояния Berth сознательно не специфицирует (комментарий в `BuildOverlay`).*

*Дополнение (2026-07, трассировка владельца к спеке v0.12) — «толщина — weight окна» уточнено: формула
перевода веса в пиксели одна для старого и нового UI (`FrameLayeredPane.setBoundsInPaletteLayer`,
`ToolWindowPane.kt:928`, `rootSize * weight`), но **источник** веса управляется настройкой
`UISettings.rememberSizeForEachToolWindowNewUI` (в new UI дефолт `false`): при дефолте
`activateToolWindow` (`ToolWindowManagerImpl.kt:582-629`) подменяет `info.weight` весом стороны
(`isUnifiedToolWindowSizesEnabled()` → `info.weight = layoutState.getUnifiedAnchorWeight(info.anchor)`)
перед каждым показом, а ресайз декоратора пишет **и** `info.weight`, **и** `unifiedWeights[anchor]`
(`ToolWindowManagerDecorators.kt:137-191`, слушатель реален — цикл замыкается). Итог: при дефолтной
конфигурации new UI толщина sliding-оверлея фактически равна **весу стороны** — оверлей ровно накрывает
докированную соседку. Спека v0.12 приняла это как единственный режим v1: поле `UndockWeight` удалено,
толщина оверлея = `Weight` стороны; пер-оконные размеры (настройка эталона) — кандидат v2 (раздел 12
спеки).*
...
докируемом типе; в спеке v0.5 — явное поле `LastInternalMode` (INV-7). Отдельно `setToolWindowAutoHide` (`:1707-1722`) переключает DockPinned↔DockUnpinned не меняя
видимости.
...
- `SetUndockWeight`: для SLIDING тот же `info.weight`, геометрию сторон не трогает. *(Команда удалена
  спекой v0.12: при дефолте new UI `info.weight` затирается весом стороны — см. дополнение к TW-3.3;
  ресайз оверлея выражается `SetSideSize`.)*
...
**TW-5.12 — `HideAll`.** `=` (спека v0.5 приняла объём IDEA)
...
«Restore Windows» (toggle) — у нас вне ядра: приложение собирает его из `Snapshot`/`Apply` (раздел 12 спеки).
...
не в этом дереве исходников. Докировка псевдоокна перетаскиванием за заголовок опирается на ту же
эквивалентность, что TW-7.8 (один `ToolWindowDragHelper` и вытаскивает панель, и докирует за заголовок —
`:805`); подавление докировки по `Ctrl` — решение владельца по UX (аналога у эталона нет, ближе всего
dock-гайды Visual Studio).
...
**TW-8.2 — список = панели без иконки, сортировка.** `≈` (спека v0.5 перешла на алфавит)
...
`toolWindowId` (`CASE_INSENSITIVE_ORDER`) (`:78-88`) — по мнемонике, затем по id алфавитно. Спека v0.5
сортирует по `Title` (tie-break `Id`); мнемоник в нашей модели нет — по существу совпадает, различие лишь
в ключе (Title vs id).
...
буквальная IDEA (с v0.5 в спеке помечено `≈ IDEA`).
...
## Раздел 11. Каталог краевых случаев (E1–E28)
...
| E27 | `internalType` + сохранение `isAutoHide` — возврат из Float в прежний внутренний режим | `=` |
| E28 | `shouldBeHiddenByShortCut = isVisible && type.isInternal` — Float/Window при HideAll не закрываются | `=` |
...
2. **SideStack** восстанавливает вытеснённую панель при закрытии заменившей (R3/E3) — у нас нет; в new UI
   IDEA выключен реестром. С v0.5 зафиксировано не-целью (раздел 12 спеки).
3. **HideAll** у IDEA не трогает Float/Window — спека v0.5 приняла объём IDEA (TW-5.12), снято.
4. Список **«⋯»**: мнемоника+алфавит у IDEA — спека v0.5 перешла на алфавит по `Title` (TW-8.2), снято.
5. Дефолт **`Weight` = 0.33** — принят спекой v0.5, снято.
6. **`sideWeight`** — одно поле у IDEA; расщепление ранних версий спеки (`CurrentRatio`+`PairRatio`) снято v0.12: остались `PairRatio` панелей, доля пары выводится нормализацией (TW-2.5/2.7).
